...
name: pp-improve
description: >
  Autonomous self-improvement agent for the entire promptplot codebase. Runs the
  full eval loop — generates test drawings, validates GCode, simulates toolpaths,
  scores results, diagnoses failures, fixes code, and iterates until quality
  improves or plateaus. The top-level improvement agent that orchestrates pp-validate,
  pp-simulate, pp-optimize, and pp-prompt together. Use when you want a hands-off
  improvement run across the whole pipeline. Triggers include "improve promptplot",
  "run the improvement agent", "make the drawings better", "fix the pipeline",
  "autonomous improvement loop", "self-improve the codebase".
---
